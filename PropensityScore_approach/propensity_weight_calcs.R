...
rm(list = c("city_lehd_acs09", "city_lehd_acs15", "city_lehd_combine_full", "city_lehd"))
...
#predicted weights restricted and non-restricted sample logit-----------
...
mfg_stabilized_full <- ipwpoint(exposure = pmd_dummy, family = "binomial", link = "logit",
                                numerator = ~1,
                                denominator = ~ mfgshare + network_density + dist_km + owner_per,
 ...
 city_lehd_acs04_restricted$ipw_ind_restrict <- ind_stable_restricted$ipw.weights
 city_lehd_acs04_restricted$ipw_mfg_restrict <- mfg_stabilized_restricted$ipw.weights
 
 
 
 
 #testing balance with cobalt package-------------
 covs1 <- city_lehd_acs04 %>% select(mfgshare, black_per, hispanic_per, renter_per, dist_km, network_density)
 
 bal.tab(covs1, treat = city_lehd_acs04$pmd_dummy, weights = city_lehd_acs04$ipw_mfg,
         method = "weighting")
 
 covs2 <- city_lehd_acs04 %>% select(indshare, black_per, hispanic_per, renter_per, dist_km, network_density)
 
 bal.tab(covs2, treat = city_lehd_acs04$pmd_dummy, weights = city_lehd_acs04$ipw_ind,
         method = "weighting")
...
prop_gt_table <- city_lehd_acs04 %>%
  select(geoid10 = bg_fips, pmd_policy, 62:72, namelsad)

prop_gt_restricted <- city_lehd_acs04_restricted %>% 
  select(geoid10 = bg_fips, pmd_policy, 62:72, namelsad)
