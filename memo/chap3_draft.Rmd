...
p_load(RPostgreSQL, ggthemes, here, ggplot2, sf, dplyr, extrafont, hrbrthemes,viridis)
...
Ten cities in the sample have what I have identified as a PMD (planned manufacturing district) type policy. In order to be identified a city had to have a separate land use classification with some form of additional protection for industrially zoned land. The archetypal example is Chicago's PMD ordinances that protect specific industrial corridors and outright ban residential conversion and require additional process steps for proposed re-zonings (see Green, 2018 for an overview). These policies vary in three main ways from one another- temporal, spatial, regulatory. First, these policies were not passed at the same time. Chicago and Portland's PMD ordinances passed in the late 1980s while the other cities generally passed their ordinances in the last 10-15 years. Spatially these policies cover different parts of their respective cities. Portland's policy covers a large swatch of industrial land in the central part of the city and on the Columbia Corridor in the northern part of the city while Baltimore's policy specifically protects port lands and other marine dependent uses. Finally, each policy has its own rules and restrictions regarding converting land to non-industrial uses. While this variation does not allow a true 1:1 comparison of policies, all of the policies make it harder to convert industrial land to non-industrial uses and so functionally all seek to reach the same goals. 
...
Due to the policy goal of protecting industrial users, manufacturing and industrial employment is significantly higher in PMD block groups than non-PMD block groups, even for areas that are heavily industrial. 
